(function(){
  ...
  if(form){
    const setInvalid = (el, msg)=>{ el.setAttribute('aria-invalid','true'); showToast(msg); };
    ...
    form.addEventListener('submit',(e)=>{
      ...
      let ok = true;
      const emailRe = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
      if(name.length < 2){ setInvalid(form.querySelector('[name="name"]'), 'Nom trop court'); ok = false; }
      else clearInvalid(form.querySelector('[name="name"]'));
      if(!emailRe.test(email)){ setInvalid(form.querySelector('[name="email"]'), 'Email invalide'); ok = false; }
      else clearInvalid(form.querySelector('[name="email"]'));
      if(message.length < 6){ setInvalid(form.querySelector('[name="message"]'), 'Message trop court'); ok = false; }
      else clearInvalid(form.querySelector('[name="message"]'));
      if(!ok) return;
      showToast('Merci ! Message envoyé — notre équipe revient vers vous sous 24h.');
      form.reset();
    });
  }
